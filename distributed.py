...
class HeatDiffusionDistributed:
    ...
    def _receive_grid_slice(self, worker_socket: socket.socket, start_row: int, end_row: int):
        ...
        Args:
            worker_socket: Socket do worker
            start_row: Primeira linha recebida
            end_row: Última linha recebida (exclusive)
        """
        # Recebe tamanho
        ...
        # Atualiza a grade (exclui linhas de borda se necessário)
        actual_start = max(1, start_row)
        actual_end = min(self.height - 1, end_row)
        slice_start = actual_start - (start_row - 1 if start_row > 0 else 0)
        slice_end = slice_start + (actual_end - actual_start)
        
        self.new_grid[actual_start:actual_end, 1:-1] = slice_data[slice_start:slice_end, 1:-1]
