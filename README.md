...
1. **Sequencial** (`sequential.py`): Processa todas as células da grade de forma sequencial
2. **Paralela** (`parallel.py`): Utiliza múltiplas threads para processar diferentes faixas da grade simultaneamente
3. **Distribuída** (`distributed.py`): Utiliza múltiplos processos comunicando-se via sockets TCP/IP
...
```bash
python distributed.py worker [host] [port]
```
...
Opções:
  ...
  --output-dir DIR           Diretório para resultados (padrão: results)
```
...
Os resultados são salvos no diretório `results/` (ou o diretório especificado):

- `benchmark_results.json`: Resultados em formato JSON
- `benchmark_results.csv`: Resultados em formato CSV
- `size_vs_time.png`: Gráfico de tempo vs tamanho do problema
...
├── parallel.py            # Implementação paralela com threads
├── distributed.py         # Implementação distribuída com sockets
├── benchmark.py           # Script de benchmark e análise
├── requirements.txt       # Dependências do projeto
├── README.md             # Este arquivo
└── results/              # Diretório de resultados (gerado)
...
seq = HeatDiffusionSequential(width, height)
seq.simulate(iterations)

par = HeatDiffusionParallel(width, height, num_threads=4)
par.simulate(iterations)

# Compara resultados (devem ser muito similares)
diff = np.abs(seq.get_grid() - par.get_grid())
